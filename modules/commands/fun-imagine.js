...
module.exports.run = async ({ api, event, args }) => {
  ...
  try {
    ...
    const response = await axios.get(`https://www.samirxpikachu.run.place/pixai?prompt=${encodeURIComponent(translation)}`);
    const imageUrl = response.data.images[Math.floor(Math.random() * response.data.images.length)];
